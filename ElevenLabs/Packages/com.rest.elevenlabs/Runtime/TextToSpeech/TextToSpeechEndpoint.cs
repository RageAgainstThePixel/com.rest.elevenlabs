...
namespace ElevenLabs.TextToSpeech
{
    ...
    public sealed class TextToSpeechEndpoint : ElevenLabsBaseEndPoint
    {
        ...
        /// <param name="cancellationToken">Optional, <see cref="CancellationToken"/>.</param>
        /// <returns>A tuple containing the <see cref="VoiceClip"/> and an array of <see cref="TimestampedTranscriptCharacter"/>.</returns>
        public async Task<(VoiceClip voiceClip, TimestampedTranscriptCharacter[] timestampedTranscriptCharacters)> TextToSpeechWithTimestampsAsync(TextToSpeechRequest request, CancellationToken cancellationToken = default)
        {
            var payload = JsonConvert.SerializeObject(request, ElevenLabsClient.JsonSerializationOptions);
            var parameters = new Dictionary<string, string>
            {
            ...
            return (voiceClip, timestampedTranscriptCharacters);
        }
        ...
        /// </param>
        /// <returns>Downloaded clip path, and the loaded audio clip.</returns>
        public async Task<(VoiceClip voiceClip, TimestampedTranscriptCharacter[] timestampedTranscriptCharacters)> StreamTextToSpeechWithTimestampsAsync(string text, Voice voice, Action<(AudioClip audioClip, TimestampedTranscriptCharacter[] timestampedTranscriptCharacters)> partialClipCallback, VoiceSettings voiceSettings = null, Model model = null, OutputFormat outputFormat = OutputFormat.PCM_24000, int? optimizeStreamingLatency = null, CancellationToken cancellationToken = default)
        ...
        public async Task<(VoiceClip voiceClip, TimestampedTranscriptCharacter[] timestampedTranscriptCharacters)> StreamTextToSpeechWithTimestampsAsync(TextToSpeechRequest request, Action<(AudioClip audioClip, TimestampedTranscriptCharacter[] timestampedTranscriptCharacters)> partialClipCallback, CancellationToken cancellationToken = default)
        {
            ...
            var response = await Rest.PostAsync(GetUrl(endpoint, parameters), payload, StreamCallback, eventChunkSize: 8192, new RestParameters(headers), cancellationToken).ConfigureAwait(true);
            response.Validate(EnableDebug);
            ...
            var downloadDirectory = await GetCacheDirectoryAsync(request.Voice);
            var cachedPath = $"{downloadDirectory}/{clipId}.ogg";
            var oggBytes = await OggEncoder.ConvertToBytesAsync(accumulatedPCMData.ToArray(), frequency, 1, cancellationToken: cancellationToken);
            await File.WriteAllBytesAsync(cachedPath, oggBytes, cancellationToken: cancellationToken);
            var fullClip = await Rest.DownloadAudioClipAsync($"file://{cachedPath}", AudioType.OGGVORBIS, parameters: new RestParameters(debug: EnableDebug), compressed: false, streamingAudio: true, cancellationToken: cancellationToken);
            var voiceClip = new VoiceClip(clipId, request.Text, request.Voice, fullClip, cachedPath);
            return (voiceClip, accumulatedTranscriptData.ToArray());
            void StreamCallback(Response partialResponse)
            {
                try
                {
            ...
            void TryFlushAccumulatedTextLines() {
                ...
                void HandleResponseJsonLine(string line) {
                    ...
                    try
                    {
                        ...
                        var pcmData = PCMEncoder.Decode(audioBytes);
                        ...
                        partialClipCallback.Invoke((audioClip, timestampedTranscriptCharacters));
